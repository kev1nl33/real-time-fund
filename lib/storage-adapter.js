...
function getGistId() {
  if (typeof window === 'undefined') return null;
  return process.env.NEXT_PUBLIC_GIST_ID || localStorage.getItem(GIST_ID_KEY) || null;
}

function setGistId(id) {
  if (typeof window === 'undefined') return;
  if (process.env.NEXT_PUBLIC_GIST_ID) return; // env var takes precedence, no need to cache
  try {
...
export async function loadFromGist() {
  ...
  try {
    ...
    if (data) {
      ...
      const gistUpdatedAt = data._updatedAt || '';
      if (gistUpdatedAt > localUpdatedAt) {
        applySyncData(data);
